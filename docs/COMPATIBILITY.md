...
| GEMM | Mature gfx906 rocBLAS/Tensile data in older releases | Rebuilds/validates Vega20 assets, returns `NOT_IMPLEMENTED` instead of aborting if data is missing, and keeps rocBLAS as the correctness fallback; FP16/FP32/FP64 are in the main smoke and INT8→INT32 `rocblas_gemm_ex` is isolated as a validate-per-kernel path |
| Sparse/solver | Legacy rocSPARSE SpMV v1 was the broadly available route | Probes ROCm 10 SpMV-v2 (two-stage analysis/compute) on gfx906 and uses v1 only as an explicit `NOT_IMPLEMENTED` fallback; rocSOLVER LU remains native-tested |
| Runtime execution | HIP streams/events and synchronous launches were the baseline | Adds a separate HIP graph-capture/instantiate/launch gate using native gfx906 code; graph support is not promoted until MI50 execution passes |
| Runtime compilation | Ahead-of-time `hipcc` was the only pre-hardware compiler check | Adds HIPRTC COMGR compilation and module-launch coverage for a real gfx906 code object; module execution remains hardware-gated |
| BLAS front end | hipBLAS could inherit a newer hipBLASLt provider on newer builds | Adds a native hipBLAS SGEMM gate with hipBLASLt explicitly disabled, proving the supported gfx906→rocBLAS dispatch once hardware is present |
| INT8 ISA primitive | Higher-level INT8 support is catalog-dependent | Adds a native GCN5.1 packed dot4 correctness gate as a fallback building block; it does not promote full INT8 GEMM or quantized LLM support by itself |
| INT8 fallback GEMM | No guaranteed ROCm 10 gfx906 INT8 catalog | Adds a correctness-only packed-dot4 INT8→INT32 GEMM fallback with scalar tail handling; performance and integration remain hardware-gated |
