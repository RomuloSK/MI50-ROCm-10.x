...
# already installed.  When a ROCm prefix is supplied, prefer its clang and
# point the image-kernel CMake code at the prefix's AMDGPU bitcode directory.
if [[ -n "${ROCM_PATH_HINT}" ]]; then
  if [[ -x "${ROCM_PATH_HINT}/lib/llvm/bin/clang" ]]; then
    export CC="${CC:-${ROCM_PATH_HINT}/lib/llvm/bin/clang}"
    export CXX="${CXX:-${ROCM_PATH_HINT}/lib/llvm/bin/clang++}"
  fi
  if [[ -z "${DEVICE_LIB_PATH}" && -d "${ROCM_PATH_HINT}/lib/llvm/amdgcn/bitcode" ]]; then
    # This option is consumed directly by the clang invocation generated by
    # ROCr's image-kernel CMake file, so pass the bitcode leaf explicitly.
    DEVICE_LIB_PATH="${ROCM_PATH_HINT}/lib/llvm/amdgcn/bitcode"
  fi
  if [[ -z "${Clang_DIR:-}" ]]; then
    for candidate in \
      "${ROCM_PATH_HINT}/lib/cmake/clang" \
      "${ROCM_PATH_HINT}/lib/llvm/lib/cmake/clang"; do
  ...
  if [[ -z "${LLVM_DIR:-}" ]]; then
    for candidate in \
      "${ROCM_PATH_HINT}/lib/cmake/llvm" \
      "${ROCM_PATH_HINT}/lib/llvm/lib/cmake/llvm"; do
