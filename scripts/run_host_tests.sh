...
if [[ -f "${ROOT_DIR}/scripts/run_mi50_graph_smoke.sh" ]]; then
  set +e
  bash "${ROOT_DIR}/scripts/run_mi50_graph_smoke.sh"
  graph_status=$?
  set -e
  if [[ "${graph_status}" -ne 0 && "${graph_status}" -ne 77 ]]; then
    exit "${graph_status}"
  fi
fi

if [[ -f "${ROOT_DIR}/scripts/run_mi50_hiprtc_smoke.sh" ]]; then
  set +e
  bash "${ROOT_DIR}/scripts/run_mi50_hiprtc_smoke.sh"
  hiprtc_status=$?
  set -e
  if [[ "${hiprtc_status}" -ne 0 && "${hiprtc_status}" -ne 77 ]]; then
    exit "${hiprtc_status}"
  fi
fi
...
if [[ -f "${ROOT_DIR}/scripts/run_mi50_int8_dot4_gemm_smoke.sh" ]]; then
  set +e
  bash "${ROOT_DIR}/scripts/run_mi50_int8_dot4_gemm_smoke.sh"
  int8_dot4_gemm_status=$?
  set -e
  if [[ "${int8_dot4_gemm_status}" -ne 0 && "${int8_dot4_gemm_status}" -ne 77 ]]; then
    exit "${int8_dot4_gemm_status}"
  fi
fi
